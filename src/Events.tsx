...
export function EventsPage() {
  return <main className="events-page">
    ...
    <section className="events-overview section-shell"><div><strong>04</strong><span>Event stories</span></div><div><strong>06+</strong><span>Campus experiences</span></div><div><strong>01</strong><span>Community spirit</span></div><a href="#event-stories">View stories <ArrowRight size={16}/></a></section>
    <section className="events-redesign section-shell" id="event-stories">
      <article className="events-featured-card"><div className="featured-copy"><p className="eyebrow">{eventGroups[0].category}</p><span className="featured-label">01</span><h2>{eventGroups[0].title}</h2><p>{eventGroups[0].description}</p><div className="event-meta"><span><CalendarDays size={15}/> Oxford Institutions</span><span><MapPin size={15}/> Hubballi Campus</span></div><a href="#event-stories">Explore moments <ArrowRight size={16}/></a></div><div className="featured-images">{eventGroups[0].images.map((image,imageIndex)=><figure key={image}><img src={image} alt={`${eventGroups[0].title} event ${imageIndex+1}`}/><figcaption><Camera size={14}/>Event moment</figcaption></figure>)}</div></article>
      <div className="event-card-grid">{eventGroups.slice(1).map((event,index)=><article className={`event-card ${event.tone}`} key={event.title}><div className="event-card-gallery">{event.images.slice(0,3).map((image,imageIndex)=><img key={`${event.title}-${imageIndex}`} src={image} alt={`${event.title} event ${imageIndex+1}`}/>)}</div><div className="event-card-copy"><span className="card-index">0{index+2}</span><p className="eyebrow">{event.category}</p><h3>{event.title}</h3><p>{event.description}</p><div className="event-meta"><span><MapPin size={14}/> Hubballi Campus</span></div><a href="#event-stories">View gallery <ArrowRight size={15}/></a></div></article>)}</div>
    </section>
